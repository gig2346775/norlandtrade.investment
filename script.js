...
/* ---- Form submit (guest — saves to Firestore) ---- */
async function submitInvestForm(e) {
  ...
  btn.innerHTML = `<span style="display:inline-flex;align-items:center;gap:10px">
    <span style="width:18px;height:18px;border:2px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:spin 0.8s linear infinite;display:inline-block"></span>
    Processing…</span>`;

  try {
    await firebase.firestore().collection('guest_investments').add({
      fullName:      document.getElementById('fi-name').value.trim(),
      ...
      accountNumber: document.getElementById('fi-account').value.trim(),
      bankName:      document.getElementById('fi-bank').value.trim(),
      submittedAt:   Date.now(),
      status:        'pending',
    });
  } catch (err) {
    console.warn('Firestore save skipped (check rules / config):', err.message);
  }
